...
transformed parameters {
  ...
  vector<lower=0>[N] poisson_rate_param; 
  ...
  for (i in 1:N) {	
    ...
    poisson_rate_param[i] = search_rate[i] * n[i];
...
model {  
  ...
  phi_d_raw ~ normal(0, 1); // we have a wider prior for phi_d than for the original threshold test. 
  
  //thresholds
  t_i ~ normal(0, sigma_t);

  s ~ poisson(poisson_rate_param);
